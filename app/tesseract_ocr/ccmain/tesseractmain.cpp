...
//int main_teseract(int argc, char **argv) 
int main_teseract(int argc, char **argv,  std::string &out_recognised_text) 
{
//  STRING outfile;               //output file
  ...
  // Save output:
  out_recognised_text=text_out.string();
...
int WINAPI WinMain(  //main for windows //command line
  ...
  if (initialized) {
    exit_code = main_teseract (argc, argv);
    free (argsin[0]);
    free (argsin[1]);
    free(argv);
    return exit_code;
